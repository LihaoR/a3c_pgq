    ...
    def __init__(self,s_size,a_size,scope,trainer,trainer_q):
        with tf.variable_scope(scope):
            ...
            if scope != 'global':
                ...
                # ac loss
                self.value_loss = tf.reduce_sum(tf.square(self.target_v - tf.reshape(self.value,[-1]))) # 0.5 ?
...
class Worker():
    ...
    def train(self,rollout,sess,gamma,lam,bootstrap_value,v1q):
        ...
        v_l,p_l,q_l,e_l,g_n,v_n,_,_ = sess.run([self.local_AC.value_loss,
            self.local_AC.policy_loss,
            self.local_AC.q_loss,
            ...
            while not coord.should_stop():
                ...
                while not d:
                    ...
                    if len(episode_buffer) == 10 and d != True:
                        ...
                        v1q = sess.run(self.local_AC.out, feed_dict={self.local_AC.inputs:[s1]})
                        v1q = np.max(v1q)
